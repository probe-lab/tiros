...
func (o *Provider) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(providerAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(providerPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert providers, could not build conflict column list")
			}

			conflict = make([]string, len(providerPrimaryKeyColumns))
			copy(conflict, providerPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"providers\"", updateOnConflict, ret, update, conflict, insert, opts...)
